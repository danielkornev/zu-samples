...
{
	internal class KeyphraseAnalyzer
	{
		internal KeyphraseAnalyzer(ISystemInformationModel SIM)
		{
			this.SIM = SIM;
		}

		internal KeyphraseAnalysis Analyze(string content)
		{
			KeyphraseAnalysis analysis = new KeyphraseAnalysis { Content = content };
			int wordCount = 0;
			...
			analysis.Keyphrases = from n in result select SIM.CreateKeyphrase(n.Key, Convert.ToDouble(n.Value));
			return analysis;
		}
		...
		public List<IKeyphrase> Extract(string text, double normalizeTo, int maxLength, out int truncateAt)
		{
			List<KeyValuePair<string, double>> results = this.ExtractAsKeyValuePair(text, normalizeTo, maxLength, out truncateAt);
			...
			{
				// all extracted keyphrases are marked as such
				keyphrases.Add(SIM.CreateKeyphrase(kvp.Key, kvp.Value, 1, EntityFragmentState.Extracted));
			}

			return keyphrases;
		}

		internal List<KeyValuePair<string, double>> ExtractAsKeyValuePair(string text, double normalizeTo, int maxLength, out int truncateAt)
		{
			var results = new List<KeyValuePair<string, double>>();

			var ka = this.Analyze(text);

			foreach (var keyphrase in ka.Keyphrases)
			{
				var kvp = new KeyValuePair<string, double>(keyphrase.Phrase, keyphrase.Rank);

				results.Add(kvp);
			}

			// we normalize to 0.99
			this.NormalizeList(results, 0.99);

			truncateAt = WeightedListTruncator<string>.ComputeCut(results, maxLength);

			return results;
		}

		internal void NormalizeList(List<KeyValuePair<string, double>> list, double normalizeTo)
		{
			double max = 0;
			for (int i = 0; i < list.Count; i++)
			{
				KeyValuePair<string, double> item = list[i];
			...
			if (0 < max)
			{
				for (int i = 0; i < list.Count; i++)
				{
					...
					KeyValuePair<string, double> keyValuePair = list[i];
					list[i] = new KeyValuePair<string, double>(key, keyValuePair.Value / max * normalizeTo);
				}
			}
		}

		internal ISystemInformationModel SIM { get; set; }
	} // class
} // namespace
