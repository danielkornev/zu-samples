...
{
	internal static class WordScraper
	{
		...
		/// <returns></returns>
		internal static List<Paragraph> ScrapeToParagraphs(string text, out int wordCount)
		{
			wordCount = 0;
			ps = new string(p);
		...
		internal static Regex RegWordCount = new Regex(@"\b\S+?\b", RegexOptions.Compiled);
		...
		// stripping out HTML tags... (Warning!)
		private static Regex non20 = new Regex(@"\b(http|https|href|script|embed|object|frameset|frame|iframe|meta|link|style|rel)\b", (RegexOptions.IgnoreCase | RegexOptions.Compiled));
		//private static Regex non20 = new Regex(@"<[^>]*>", (RegexOptions.IgnoreCase | RegexOptions.Compiled));
		// from http://www.andrewdenhertog.com/c/strip-html-tags-using-regular-expressions-regex-in-c/
